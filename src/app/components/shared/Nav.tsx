...
import { Bell, Home, Briefcase, Calendar, MessageCircle, User, DollarSign, LogOut, Wrench, Users, BarChart2, Settings, AlertTriangle } from "lucide-react";
...
export function CustomerNav({ dark, toggleDark }: { dark: boolean; toggleDark: () => void }) {
  ...
  const links = [
    ...
    { label: "Find Workers", path: "/app/discover" },
    { label: "My Bookings",  path: "/app/booking" },
  ...
  return (
    <nav className="sticky top-0 z-50 bg-card border-b border-border px-6 py-3 flex items-center gap-6">
      ...
      <div className="hidden md:flex items-center gap-1 ml-4 flex-1">
        {links.map(({ label, path }) => (
          <button key={label} onClick={() => navigate(path)}
            className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${location.pathname === path ? "text-white" : "text-muted-foreground hover:text-foreground hover:bg-muted"}`}
            style={location.pathname === path ? { backgroundColor: A } : {}}>{label}</button>
        ))}
      </div>
      <div className="flex items-center gap-2 ml-auto">
        ...
        <button className="w-8 h-8 rounded-full text-white flex items-center justify-center text-xs font-bold" style={{ background: A }}>AR</button>
      </div>
    </nav>
  );
}
    ...
    <aside className="w-64 shrink-0 flex flex-col h-screen sticky top-0" style={{ backgroundColor: P }}>
      ...
      <nav className="flex-1 p-3 space-y-1">
        {items.map(({ icon, label, path }) => (
          <button key={label} onClick={() => navigate(path)}
            className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all ${location.pathname === path ? "bg-white/20 text-white" : "text-white/70 hover:text-white hover:bg-white/10"}`}>
            {icon}{label}
          </button>
        ))}
      </nav>
    ...
    <aside className="w-64 shrink-0 flex flex-col h-screen sticky top-0" style={{ backgroundColor: P }}>
      ...
      <nav className="flex-1 p-3 space-y-1">
        {items.map(({ icon, label }) => (
          <button key={label} onClick={() => navigate("/admin")}
            className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all ${location.pathname === "/admin" && label === "Overview" ? "bg-white/20 text-white" : "text-white/70 hover:text-white hover:bg-white/10"}`}>
            {icon}{label}
          </button>
        ))}
      </nav>
