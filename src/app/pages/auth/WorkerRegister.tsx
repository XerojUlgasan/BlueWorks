...
import { P, BARANGAYS } from "../../constants";
...
export default function WorkerRegister() {
  ...
  async function handleRegister() {
    ...
    const { error: err } = await supabase.auth.signUp({
      email, password,
      options: { data: { full_name: name, role: "worker", phone, address, barangay } },
    });
    setLoading(false);
  ...
  if (done) {
    return (
      <AuthCard label="Almost there!" portalType="worker">
        <div className="text-center space-y-4 py-4">
          <div className="w-16 h-16 rounded-full mx-auto flex items-center justify-center" style={{ background: `${P}20` }}>
          ...
          <button onClick={handleResend} disabled={cooldown > 0}
            className="text-sm font-semibold disabled:opacity-50 disabled:cursor-not-allowed" style={{ color: P }}>
            {cooldown > 0 ? `Resend in ${cooldown}s` : "Resend confirmation email"}
          </button>
          {error && <p className="text-sm text-red-500">{error}</p>}
          <button onClick={() => navigate("/worker/login")} className="w-full py-2.5 rounded-lg border border-border text-sm font-medium hover:bg-muted transition-colors mt-2">
            Back to Login
          </button>
        </div>
      </AuthCard>
  ...
  return (
    <AuthCard label="Create your account" portalType="worker">
      <div className="space-y-3">
        <InputField label="Full Name" placeholder="Juan dela Cruz" value={name} onChange={setName} />
        <InputField label="Contact Number" placeholder="+63 917 123 4567" value={phone} onChange={setPhone} />
        <InputField label="Email Address" type="email" placeholder="juan.delacruz@email.com" value={email} onChange={setEmail} />
        ...
        <button onClick={handleRegister} disabled={loading}
          className="w-full py-3 rounded-lg text-white font-semibold mt-2 transition-opacity hover:opacity-90 disabled:opacity-60" style={{ background: P }}>
